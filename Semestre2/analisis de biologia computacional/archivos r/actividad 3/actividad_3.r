...
if(tolower(selection) == "m"){ #si selecciona m --> manual
  ...
  #search <- EUtilsSummary("Covid19[TI] OR (coronavirus[TI] AND pandemic[TIAB])")
...
title_words <- unlist(strsplit(tolower(title), "[ ,.:()]"))
...
for(i in 0:nrow(df)){
  ...
  abstracts <-paste(abstracts, words, sep = "")
}
#Se guardan los abstracts concatenados como txt
...
#abstract_words <- unlist(strsplit(tolower(abstract), "[ ,.:!??()]"))
abstract_words <- unlist(strsplit(tolower(abstract), "[ ,.:()]"))
...
cat("Number of articles found: ", length(df[[1]]))
...
#se grafica
barplot(frequency_title, main="Palabras más frecuentes titulos", names.arg=word_list_title, cex.names=1.5, col = grey.colors(10), cex.names = 0.5)
...
#se grafica
barplot(frequency_abstract, main="Palabras más frecuentes abstracts", names.arg=word_list_asbtract, cex.names=1.5, col = grey.colors(10))
