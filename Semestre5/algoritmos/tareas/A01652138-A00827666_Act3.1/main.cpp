...
bool Trie::haveChildren(Trie const* curr){
    for (int i = 0; i < CHAR_SIZE; i++){
        /* code */
        if(curr->character[i]) return true; //se ecnontro un hijo
...
int main(){
    ...
    //insertar en el trie
    for(int i = 0; i < sizeof(words)/sizeof(words[0]);i++) head->insert(words[i]);
    

    cout<<"----------------------------------------------------------------------------------------\n";
    cout<<"Ingresar numero de palabras a buscar >\n";cin>>m;

    //se almacenan las palabras
    string searchWords[m];
    for(int i = 0;i<m;i++) cin>>searchWords[i];
    
    cout << "\n";
    
    //se buscan las palabras
    for(int i = 0;i<m;i++) cout<<head->search(searchWords[i],true)<<endl;
