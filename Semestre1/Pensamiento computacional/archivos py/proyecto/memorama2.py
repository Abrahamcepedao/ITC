...
def create_game():
    #matriz unidemensional con los países y capitales
    ...
    #inicializar matriz bidimensional que contendrá los países y capitales ya ordenados de manera aleatoria
    game = [[0] * 6 for _ in range(6)]
    count = -1
    #for para asignarle un valor de la primera matriz a cada valor de la segunda matriz
    for ex in range(6):
        for ey in range(6):
            count += 1
...
def get_number(board):
    #recibe los números y saca su posición
...
def display_player_board(board, game, num1, num2, row, col, row2, col2, ptsP1, ptsP2, player_counter):
    #actualizar el tablero de manera 
...
def main():
    ...
    print(f'Points player1: {ptsP1} \nPoints player2: {ptsP2}')
    if ptsP1 > ptsP2:
